...
class MRINet(nn.Module):
    def __init__(self):
        ...
        # The keys in the state_dict indicate that the model was saved with a Dropout layer
        # followed by a Linear layer in a Sequential container.
        self.base.fc = nn.Sequential(
            nn.Dropout(0.5), # Re-add the dropout layer
...
def load_model():
    ...
    model = MRINet()
    model.load_state_dict(torch.load('mri_classifier.pth', map_location=torch.device('cpu')))
    model.eval()
    return model
